...
async def create_record(table_name: str, data = Dict[str, Any], db=Depends(connect_to_db)):
...
@app.get('/{table_name}/{record_id}/', response_model=Dict[str, Any])
def get_record(table_name: str, record_id: int, db=Depends(connect_to_db)):
    if table_name not in models_db:
        raise HTTPException(status_code=404, detail=f'Table {table_name} not found!')
    model = models_db[table_name]
...
async def create_record(table_name: str, data: Dict[str, Any], db=Depends(connect_to_db)):
    ...
    return validation_data(record, [field.name for field in model._meta.sorted_fields])

@app.put('/{table_name}/{record_id}/', response_model=Dict[str, Any])
async def update_record(table_name: str, record_id: int, data: Dict[str, Any], db=Depends(connect_to_db)):
    if table_name not in models_db:
        raise HTTPException(status_code=404, detail=f'Table {table_name} not found!')
    model = models_db[table_name]
...
@app.delete('/{table_name}/{record_id}/', response_model=Dict[str, Any])
async def delete_record(table_name: str, record_id: int, db=Depends(connect_to_db)):
    if table_name not in models_db:
        raise HTTPException(status_code=404, detail=f'Table {table_name} not found!')
    model = models_db[table_name]
